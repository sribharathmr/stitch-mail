...
export default function ComposeModal() {
  ...
  const { user } = useAuth()
  const [to, setTo] = useState(composeDraft.to?.length ? composeDraft.to : [])
  const [toInput, setToInput] = useState('')
  const [cc, setCc] = useState(composeDraft.cc?.length ? composeDraft.cc : [])
  const [bcc, setBcc] = useState(composeDraft.bcc?.length ? composeDraft.bcc : [])
  const [showCc, setShowCc] = useState(composeDraft.cc?.length > 0)
  const [showBcc, setShowBcc] = useState(composeDraft.bcc?.length > 0)
